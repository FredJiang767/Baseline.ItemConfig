...
public class ItemService
{
    ...
    public async Task<IEnumerable<ItemReadDto>> GetAll()
    {
        ...
        return items.Select(x => new ItemReadDto(x.ItemId, x.ItemYear, x.ItemNumber, x.UiSubTabId, x.UiTabId));
    }

    public async Task<ItemReadDto> Create(int itemYear, string itemNumber, Guid uiSubTabId, Guid uiTabId)
    {
        ...
        var item = Item.Create(itemYear, itemNumber, uiSubTabId, uiTabId);
        _itemRepository.Add(item);
        await _uow.SaveChangesAsync();
        return new ItemReadDto(item.ItemId, item.ItemYear, item.ItemNumber, item.UiSubTabId, item.UiTabId);
    }
